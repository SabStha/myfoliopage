<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-data="{
                activeTab: '{{ $editingBlog ? 'edit' : 'list' }}',
                showCreateForm: false,
                showImportForm: false,
                editingBlog: @js($editingBlog ? $editingBlog->id : null),
                init() {
                ...
                editBlog(blogId) {
                    window.location.href = '{{ route('admin.blogs.index') }}?edit=' + blogId;
                }
            }">
                <!-- Tabs -->
                <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="border-b border-gray-200 dark:border-gray-700">
                        <nav class="flex -mb-px">
                            ...
                            @if($editingBlog)
                            <button @click="switchTab('edit')" :class="activeTab === 'edit' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="px-6 py-4 text-sm font-medium border-b-2 transition-colors">
                                ✏️ Edit Blog
                            </button>
                            @endif
                        </nav>
                                ...
                                <tbody>
                                    @forelse($blogs as $blog)
                                        <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                            <td class="py-3 px-2">
                                                <button @click="editBlog({{ $blog->id }})" class="text-blue-600 hover:underline font-medium">
                                            ...
                                            <td class="py-3 px-2 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    @endif
                                                    <button @click="editBlog({{ $blog->id }})" class="text-blue-600 hover:underline text-sm">Edit</button>
                ...
                <div x-show="activeTab === 'edit'" class="space-y-6">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900 dark:text-gray-100">
                            <h3 class="text-lg font-semibold mb-4">Edit Blog Post</h3>
                            
                            <form method="POST" action="{{ route('admin.blogs.update', $editingBlog) }}" enctype="multipart/form-data" class="space-y-4">
                                @csrf
                                @method('PUT')
                                <div>
                                    <label class="block text-sm mb-1">Title <span class="text-red-500">*</span></label>
                                ...
                                <div>
                                    ...
                                    <input name="slug" value="{{ old('slug', $editingBlog->slug) }}" class="w-full rounded border-gray-300" required />
                                    @error('slug')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <x-dual-language-input 
                                        name="excerpt" 
                                        label="Excerpt" 
                                        :value="$editingBlog->getTranslations('excerpt')"
                                        rows="3"
                                    />
                                    @error('excerpt')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm mb-1">Category</label>
                                        <input name="category" value="{{ old('category', $editingBlog->category) }}" class="w-full rounded border-gray-300" placeholder="e.g., Laravel, Frontend" />
                                        @error('category')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label class="block text-sm mb-1">Published At</label>
                                        <input type="datetime-local" name="published_at" value="{{ old('published_at', optional($editingBlog->published_at)->format('Y-m-d\TH:i')) }}" class="w-full rounded border-gray-300" />
                                        @error('published_at')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div>
                                    <label class="block text-sm mb-1">LinkedIn Post URL (optional)</label>
                                    <input type="url" name="linkedin_url" value="{{ old('linkedin_url', $editingBlog->linkedin_url) }}" class="w-full rounded border-gray-300" placeholder="https://www.linkedin.com/feed/update/..." />
                                    <p class="text-xs text-gray-500 mt-1">Add the LinkedIn post URL if this blog is related to a LinkedIn post</p>
                                    @if($editingBlog->linkedin_url)
                                        <p class="text-xs text-blue-600 mt-1">
                                            <a href="{{ $editingBlog->linkedin_url }}" target="_blank" class="hover:underline">🔗 View LinkedIn Post</a>
                                        </p>
                                    @endif
                                    @error('linkedin_url')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                ...
                                <div>
                                    <label class="block text-sm mb-1">Cover Image</label>
                                    @if($editingBlog->media->where('type', 'image')->first())
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $editingBlog->media->where('type', 'image')->first()->path) }}" alt="Current cover" class="w-32 h-32 object-cover rounded border" />
                                ...
                                <div>
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', $editingBlog->is_published) ? 'checked' : '' }} />
                                ...
                                <div class="flex justify-end gap-2">
                                    <button type="button" @click="switchTab('list')" class="px-4 py-2 text-sm rounded border">Cancel</button>
                                    <button type="submit" class="px-4 py-2 text-sm rounded bg-blue-600 text-white">Save Changes</button>
                                </div>
                            </form>
                        </div>
                    </div>
